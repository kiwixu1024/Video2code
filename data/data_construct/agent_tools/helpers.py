...
def get_current_img_b64(port):
    r = requests.get(f"http://localhost:{port}/observe_sized", proxies={"http": None, "https": None}, timeout=120).json()
    return r["image_b64"]
...
def set_page_state(state, port, t0: float | None = None, label_for_moments: str | None = None):
    ...
    """
    requests.post(f"http://localhost:{port}/reset", proxies={"http": None, "https": None}, timeout=120).json()
    per_step_moments = []
    ok = True
    shift_index = 1
    for step in state["actions"]:
        action = step["action"]
        dt_info = requests.get(f"http://localhost:{port}/dom_tree_with_id", proxies={"http": None, "https": None}, timeout=120).json()
        id2xpath = dt_info["id2xpath"]
        time.sleep(1)
        t1 = time.perf_counter()
        if action == "click":
            resp = requests.post(f"http://localhost:{port}/click",
                                 json={"id": step["id"], "id2xpath": id2xpath}, proxies={"http": None, "https": None}, timeout=180).json()
        elif action == "input":
            resp = requests.post(f"http://localhost:{port}/enter",
                                 json={"id": step["id"], "text": step["input_val"], "id2xpath": id2xpath}, proxies={"http": None, "https": None}, timeout=180).json()
        elif action == "select":
            resp = requests.post(f"http://localhost:{port}/select",
                                 json={"id": step["id"], "value": step["value"], "id2xpath": id2xpath}, proxies={"http": None, "https": None}, timeout=180).json()
        else:
            resp = True
