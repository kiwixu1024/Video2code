...
class WebHtmlGymEnv:
    ...
    async def _smooth_scroll_into_view(
        ...
        def _clipped_center(inf):
            # Center of the visible portion of the element, guaranteed within viewport.
            vis_top = max(inf["top"], margin_px)
            vis_bottom = min(inf["bottom"], inf["vh"] - margin_px)
            cx = inf["left"] + inf["width"] / 2
            cy = (vis_top + vis_bottom) / 2
            cx = max(1, min(inf["vw"] - 1, cx))
            cy = max(1, min(inf["vh"] - 1, cy))
            return (cx, cy)

        def is_centered(inf):
            # Element's geometric center is in the middle 60% of the viewport so that
            # clicking it will hit the element, not just a barely-visible sliver at the edge.
            if inf["height"] <= 0 or inf["width"] <= 0:
                return False
            elem_cy = inf["top"] + inf["height"] / 2
            low = inf["vh"] * 0.45
            high = inf["vh"] * 0.55
            return (
        ...
        def is_visible(inf):
            ...
            return (
                inf["height"] > 0 and inf["width"] > 0 and
                inf["bottom"] > margin_px and
                inf["top"] < inf["vh"] - margin_px
            )

        if is_centered(info):
            return _clipped_center(info)

        prev_scroll_y = await self.page.evaluate("() => window.scrollY")
        ...
        for _ in range(max_rounds):
            ...
            # If the page didn't scroll (document boundary reached) and the element
            # is visible, centering is impossible — return the best available position.
            curr_scroll_y = await self.page.evaluate("() => window.scrollY")
            if curr_scroll_y == prev_scroll_y and is_visible(info):
                return _clipped_center(info)
            prev_scroll_y = curr_scroll_y

        # Exhausted retries — fall back to clipped center if still visible.
        info = await self._element_view_info(elem)
        if is_visible(info):
            return _clipped_center(info)
        return None
...
#DOM TREE 
        ...
        async with self._page_lock:
            ...
            () => {
                ...
                function isInteractiveElement(node) {
                    ...
                    // --- 1. 原版的判断逻辑完全保留 ---
                    if (['button', 'a', 'select', 'textarea', 'label', 'i'].includes(tag)) return true;
                    if (tag === 'input') {
    ...
    async def _scroll_page_human_like_internal(self, bar_color="#00cc00"):
        ...
        try:
            ...
            estimated_ms = int(max_scroll * 0.9 + 500)
            ...
            await self.page.evaluate(js_code, estimated_ms)
    ...
    async def scroll_to_top(self):
        """Instantly scroll the page back to the very top (0, 0)."""
        async with self._page_lock:
            try:
                await self.page.evaluate("() => window.scrollTo(0, 0)")
                self.click_history.append(("scroll_to_top", None))
                return True
            except Exception as e:
                print(f"[SCROLL] scroll_to_top failed: {e}")
                return False

    async def scroll_to_element(self, unique_id, id2xpath: dict):
        """
        Scroll to the element identified by unique_id using human-like animation.
        If unique_id is None, scroll all the way to the bottom.
        """
        async with self._page_lock:
            return await self._scroll_to_element_internal(unique_id, id2xpath)

    async def _scroll_to_element_internal(self, unique_id, id2xpath: dict) -> bool:
        try:
            if self.page.is_closed():
                return False

            if unique_id is None:
                await self._scroll_page_human_like_internal()
                return True

            xpath = id2xpath.get(str(unique_id)) or id2xpath.get(int(unique_id))
            if not xpath:
                print(f"[SCROLL] No xpath found for id={unique_id}")
                return False

            elem = await self.page.query_selector(f'xpath={xpath}')
            if elem is None:
                print(f"[SCROLL] No element found by xpath {xpath} for id={unique_id}")
                return False

            elem_top = await self.page.evaluate(
                "(el) => el.getBoundingClientRect().top + window.scrollY",
                elem,
            )
            elem_height = await self.page.evaluate(
            ...
            target_y = elem_top - (viewport_height - elem_height) / 2
            target_y = max(0.0, min(float(target_y), float(max_scroll)))

            current_y = await self.page.evaluate("() => window.scrollY")
            if abs(target_y - current_y) < 5:
                self.click_history.append(("scroll_to_element", unique_id))
                return True
            ...
            async (targetY) => {
                ...
                const durationMs = Math.max(200, Math.min(1200, absDist * 0.6));
                const startTime = performance.now();
            ...
            """
            await self.page.evaluate(js_code, target_y)
            self.click_history.append(("scroll_to_element", unique_id))
            return True

        except Exception as e:
            print(f"[SCROLL] scroll_to_element failed: {e}")
            return False
        ...
        async with self._page_lock:
            ...
            try:
                ...
                try:
                    ...
                    print("[CLICK] Click failed, encounters an Exception")
                    pass
            ...
            async def _start_screencast():
                try:
                    ...
                    await self._cdp_session.send("Page.startScreencast", {
                        "format": "jpeg",
                        "quality": 80,
                        "everyNthFrame": 15 # 建议改小一点，比如5，保证流畅度
                    })
...
@app.post("/scroll")
async def api_scroll(req: ScrollRequest):
    global env
    start = time.perf_counter()
    result = await env.scroll_to_element(req.id, req.id2xpath)
    t_start = None
    return {"result": result, "time": start, "in_time": t_start}


@app.post("/scroll_to_top")
async def api_scroll_to_top():
    global env
    result = await env.scroll_to_top()
