...
class WebHtmlGymEnv:
    ...
    async def _debug_bar_init(self):
        """
        确保页面底部有一条常驻蓝色调试条（不会删除，只会改颜色）。
        """
        js = """
        () => {
          let bar = document.getElementById('___webenv_debugbar');
          if (!bar) {
            bar = document.createElement('div');
            bar.id = '___webenv_debugbar';
          ...
          // 默认 idle 状态：蓝色
          bar.style.background = '#0044ff';

          // 记录一个全局状态对象，用于后续清理定时器
          if (!window.__webenvDebugBarState) {
            window.__webenvDebugBarState = { timer: null };
          } else if (window.__webenvDebugBarState.timer) {
            clearTimeout(window.__webenvDebugBarState.timer);
            window.__webenvDebugBarState.timer = null;
          }
    ...
    async def _debug_bar_pulse(self, label: str = "EVENT", active_ms: int = 160):
        ...
        ({ label, activeMs }) => {
          ...
          const S = (window.__webenvDebugBarState = window.__webenvDebugBarState || { timer: null });
          ...
          // 这里你要不要显示文字都可以，如果只想要纯色条，可以注释掉 textContent
          // bar.textContent = label || 'EVENT';
    ...
    async def _flash_blue_frames(self, frames: int = 30):
        duration_sec = frames / 60.0

        # ---- 每次调用前：删除旧蓝屏 ----
        js_remove_old = """
        () => {
            const old = document.getElementById('___webenv_blue');
            if (old) old.remove();
        }
        """
        ...
        js_hide = """
        () => {
            const ov = document.getElementById('___webenv_blue');
            if (ov) ov.remove();
        }
        """
        ...
        try:
            # 删除旧蓝屏（无论是否存在）
            await self.page.evaluate(js_remove_old)

            # 创建新的蓝屏
            await self.page.evaluate(js_show)

            # 强制刷新渲染
            await self.page.screenshot(type='png', clip={'x':0, 'y':0, 'width':1, 'height':1})

            # 停留
            await asyncio.sleep(duration_sec)

            # 删除蓝屏
            await self.page.evaluate(js_hide)

        except Exception as e:
            print(f"[flash_blue] {e}")

    async def _flash_blue_cdp(self, frames: int = 30):

        duration_sec = frames / 60.0
        
        js_canvas = """
        () => {
            const id = '___blue_canvas_overlay';
        ...
        js_remove = "() => document.getElementById('___blue_canvas_overlay')?.remove()"

        await self.page.evaluate(js_canvas)
        # 同样，踢一脚渲染引擎
        await self.page.screenshot(type='png', clip={'x':0, 'y':0, 'width':1, 'height':1})
        await asyncio.sleep(duration_sec)
        await self.page.evaluate(js_remove)
    async def _inject_cursor_overlay(self):
        """在页面注入 PNG 鼠标图案（从同目录读取 cursor.png）。"""
        import base64
        import pathlib

        # 找到 cursor.png（与你运行的 main py 在同目录）
        png_path = pathlib.Path(__file__).parent / "cursor.png"
        if not png_path.exists():
            print("[WARN] cursor.png 未找到，无法注入鼠标图标")
            return
    ...
    async def _recover_mouse_and_ui(self, prev_mouse: Optional[tuple[float, float]]):
        """
        在一次操作结束后复原鼠标和 UI 叠加层：
        - 重新注入鼠标 overlay 和 debug bar
        - 尽量把鼠标恢复到操作前的屏幕坐标（如果不在当前 viewport，就放到中心）
        """
        try:
            await self._inject_cursor_overlay()
            await self._debug_bar_init()
        except Exception as e:
            print(f"[recover_ui] inject error: {e}")

        # 拿当前 viewport 尺寸（先用 Playwright 的 viewport_size，退化到 window.innerWidth/Height）
        try:
            vp = self.page.viewport_size
            if not vp:
                vp = await self.page.evaluate(
            ...
            print(f"[recover_ui] viewport error: {e}")
        ...
        if prev_mouse is not None:
            ...
            if 0 <= px < vw and 0 <= py < vh:
                ...
                # 原位置已经不在当前 viewport（或者数值异常），直接回中心
                tx, ty = vw // 2, vh // 2
        else:
            tx, ty = vw // 2, vh // 2
        ...
        try:
            ...
            print(f"[recover_ui] move error: {e}")
    ...
    async def setup(self):
        ...
        await self._inject_cursor_overlay()
        await self._debug_bar_init()  # 🌟 初始化底部调试条
    ...
    async def reset(self):
        ...
        await self._inject_cursor_overlay()
        await self._debug_bar_init()  # 🌟 每次 reset 都保证调试条存在
    ...
    async def click(self, unique_id: int, id2xpath: dict):
        ...
        try:
            ...
            # 5) 执行真实鼠标点击（down→up）
            # await self._flash_blue_frames(30)
            await self._debug_bar_pulse("CLICK", active_ms=160)  # 🌟 底部调试条变红
    ...
    async def enter(self, unique_id, content, id2xpath):
        ...
        try:
            ...
            # await self._flash_blue_frames(30)
            # 点击元素以聚焦
            await self._debug_bar_pulse("ENTER", active_ms=160)  # 🌟 底部调试条变红
    ...
    async def select(self, unique_id: Any, value: Any, id2xpath: Dict[Any, str]) -> bool:
        ...
        try:
            ...
            # 和 click 同步：预置一次性 Promise，解析在“点击后的下一帧”
            await self.page.evaluate("""
                () => {
                    let resolveFn;
                    window.__webenvClickFrame = {
            ...
            await self._move_mouse_uniform_to(tx, ty)
            # await self._flash_blue_frames(30)
            await self._debug_bar_pulse("SELECTT", active_ms=160)  # 🌟 底部调试条变红
